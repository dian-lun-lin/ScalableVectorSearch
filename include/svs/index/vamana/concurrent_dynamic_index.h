...
///
/// Only used for `ConcurrentMutableVamanaIndex`.
///
...
class ConcurrentMutableVamanaIndex {
  public:
    // Traits
  ...
  public:
    ...
    // keep the same API
    template <typename I, typename Query>
    void search(
        ...
        auto scratch = extensions::single_search_setup(data_, distance_);
    ...
    size_t add_points(
        ...
        if(s > status_.size()) {
            ...
            status_.resize(new_size, SlotMetdata::Empty);
...
auto auto_dynamic_assemble(
    ...
    // Construct the index!
    return ConcurrentMutableVamanaIndex{
        parameters,
        std::move(data),
        std::move(graph),
        std::move(distance),
